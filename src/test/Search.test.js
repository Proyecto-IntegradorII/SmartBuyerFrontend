...
const { getByPlaceholderText, getByText, getByDisplayValue, queryByText, getByRole } = render(
	<Router>
		<Search />
	</Router>
);
...
test("handles input change correctly", () => {
	const textarea = getByPlaceholderText(
		"Azúcar blanca 2.5kg, Café Aguila Roja 500 gramos, Leche bolsa 6 unidades 1 ml Alpina..."
	);
...
/*test('sends audio recording to server and updates search input', async () => {
  ...
  await waitFor(() => {
    expect(fetchMock).toHaveBeenCalledWith(
      'https://smartbuyerbackend-production.up.railway.app/transcribes',
      expect.any(Object)
    );
  });
