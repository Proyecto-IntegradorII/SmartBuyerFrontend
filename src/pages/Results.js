...
const Results = () => {
  ...
  return (
    <div className="container mx-auto p-4 font-text">
      ...
      <h1 className="text-2xl font-[750] mt-12 mb-16 text-[#e29500]">Encontramos los siguientes productos de acuerdo a tus necesidades</h1>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4 border border-[#e29500] rounded-2xl p-8">
              {selectedProducts[productIndex].map((item, index) => {
                const discountPercentage = item.priceWithoutDiscount
                  ? Math.round(((item.priceWithoutDiscount - item.priceDiscounted) / item.priceWithoutDiscount) * 100)
                  : null;
                return (
                  <div key={index} className="text-center">
                    <div className='flex justify-center'>
                      <div>
                        ...
                        <div className={`font-bold ${index === lowestPriceIndex ? 'text-green-700' : ''} ${index === lowestPriceIndex ? 'text-lg' : 'text-base'}`}>
                          ${item.priceDiscounted} {discountPercentage !== null && (
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className='flex flex-col justify-center items-center'>
            <div
              className={`image-box sm:w-28 sm:h-28 bg-center bg-cover rounded-3xl shadow-lg ${calculateTotal(0) === Math.min(calculateTotal(0), calculateTotal(1), calculateTotal(2)) ? 'border-green-500' : ''}`}
              style={{ backgroundImage: "url(/images/images.png)" }}
            >
              <div className="image-name hidden">Exito</div>
            </div>
            <div className={`font-bold my-4 ${calculateTotal(0) === Math.min(calculateTotal(0), calculateTotal(1), calculateTotal(2)) ? 'text-green-700 text-lg' : 'text-base'}`}>
              ${calculateTotal(0)}
            </div>
          </div>
          <div className='flex flex-col justify-center items-center'>
            <div
              className={`image-box sm:w-28 sm:h-28 bg-center bg-cover rounded-3xl shadow-lg ${calculateTotal(2) === Math.min(calculateTotal(0), calculateTotal(1), calculateTotal(2)) ? 'border-green-500' : ''}`}
              style={{ backgroundImage: "url(/images/jumbo2.jpg)" }}
            >
              <div className="image-name hidden">Jumbo</div>
            </div>
            <div className={`font-bold my-4 ${calculateTotal(1) === Math.min(calculateTotal(0), calculateTotal(1), calculateTotal(2)) ? 'text-green-700 text-lg' : 'text-base'}`}>
              ${calculateTotal(1)}
            </div>
          </div>
          <div className='flex flex-col justify-center items-center'>
            <div
              className={`image-box sm:w-28 sm:h-28 bg-center bg-cover rounded-3xl shadow-lg ${calculateTotal(1) === Math.min(calculateTotal(0), calculateTotal(1), calculateTotal(2)) ? 'border-green-500' : ''}`}
              style={{ backgroundImage: "url(/images/Olimpical.png)" }}
            >
              <div className="image-name hidden">Olimpica</div>
            </div>
            <div className={`font-bold my-4 ${calculateTotal(2) === Math.min(calculateTotal(0), calculateTotal(1), calculateTotal(2)) ? 'text-green-700 text-lg' : 'text-base'}`}>
              ${calculateTotal(2)}
            </div>
          </div>
        </div>
